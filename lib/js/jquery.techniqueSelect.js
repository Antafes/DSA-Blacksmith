...
$.widget('dsabs.techniqueSelect', {
    ...
    _destroy: function () {
        $(this.options.addLink).off('click.ts');
        this.clear();
        this.options.templates.select.children().remove();
    },
    addRow: function (technique) {
        var row;
        this._addRow();
        row = this.element.children(this.options.target).children('tr:last');
        row.children('.technique').children().val(technique);
    },
    clear: function () {
        this.element.children(this.options.target).find('[class=' + this.options.templates.removeLink.attr('class') + ']').trigger('click.ts');
